...
async function fetchWikipediaHtml(slug) {
  ...
  const response = await fetch(url, {
    headers: {
      'accept': 'text/html; charset=UTF-8; profile="https://www.mediawiki.org/wiki/Specs/HTML/2.7.0"',
  ...
  if (!response.ok) {
    ...
    const error = new Error(`wikipedia fetch failed: ${response.status} ${message}`);
